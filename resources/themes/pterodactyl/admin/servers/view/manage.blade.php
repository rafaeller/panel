        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">Server Transfer in Progress</h3>
                </div>
                <div class="box-body">
                    <div class="progress">
                        {!! $transfer['progress_bar'] !!}
                    </div>
                    <p>This server is a(n) <b>{{$transfer['type']}}</b> server. Current status: <code>{{$transfer['status']}}</code></p>
                </div>
            </div>
        </div>
    </div>
...
<div class="row">
	...
	<div class="box box-info">
		<form action="{{ route('admin.servers.view.manage.transfer', $server->id) }}" method="POST">
			<div class="box-header with-border">
				<h3 class="box-title">Server Transfer</h3>
			</div>
			<div class="box-body">
				<div class="form-group">
					<label for="new_node">New Node</label>
					<select id="new_node" name="new_node" class="form-control" onchange="onChange();">
						@foreach($nodes as $node)
							@if ($node->id == $server->node_id)
								<option selected value="{{$node->id}}">{{$node->name}} (Current)</option>
							@else
				...
				<div class="form-group">
					<label for="new_allocation">New Allocation</label>
					<select id="new_allocation" name="new_allocation" class="form-control"></select>
				</div>
			</div>
			<div class="box-footer">
				{!! csrf_field() !!}
				<button type="submit" class="btn btn-success">Transfer Server</button>
			</div>
		</form>
	</div>
</div>
...
@section('footer-scripts')
    @parent
    <script>
        let allocations = @json($allocations, JSON_PRETTY_PRINT);
        ...
        function onChange() {
            ...
            allocations.forEach(function (item) {
                ...
                if (item['node_id'] == node) {
                    $('#new_allocation').append('<option value="' + item['id'] + '">' + item['ip'] + ':' + item['port'] + '</option>');
                }
            });
        }
